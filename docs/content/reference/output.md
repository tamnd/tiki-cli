...
describes what users get. Pick a format with `-o`, or let tiki choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tiki <command> -o table   # aligned columns for reading
tiki <command> -o jsonl   # one JSON object per line, for piping
tiki <command> -o json    # a single JSON array
tiki <command> -o csv     # spreadsheet friendly
tiki <command> -o tsv     # tab-separated
tiki <command> -o url     # just the URL column
tiki <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tiki <command> --fields id,title,url
```
...
```bash
tiki <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tiki <command>            # a table, because this is a terminal
tiki <command> | wc -l    # JSONL, because this is a pipe
```
